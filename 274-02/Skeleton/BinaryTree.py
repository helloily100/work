...
class BinaryTree:
    ...
    def traverse(self, u : Node):
        if u == self.nil:
            return self.traverse(u.left), self.traverse(u.right)
    ...
    def in_order(self, u: Node) :
        if u.left != self.nil:
            self.in_order(u.left)
        print(f"{u.x}: {u.v}")
        if u.right != self.nil:
            self.in_order(u.right)

    def pre_order(self, u: Node) :
        print(f"{u.x}: {u.v}")
        if u.left != self.nil:
            self.pre_order(u.left)
        if u.right != self.nil:
            self.pre_order(u.right)

    def post_order(self, u: Node) :
        if u.left != self.nil:
            self.post_order(u.left)
        if u.right != self.nil:
            self.post_order(u.right)
        print(f"{u.x}: {u.v}")
